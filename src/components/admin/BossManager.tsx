...
export default function BossManager() {
 ...
 return (
 <div className="glass rounded-[20px] p-5"> <h2 className="font-display font-semibold text-ink">จัดการบอส</h2> <p className="mt-1 text-xs text-ink/50">อัปโหลดรูปมอนสเตอร์เองแทนอิโมจิได้ (สูงสุด 3MB ต่อรูป)</p> <ul className="mt-4 divide-y divide-black/5">
 {bosses.map((boss) => (
 <li key={boss.id} className="flex items-center gap-4 py-3"> <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-black/5 text-3xl">
 {boss.icon_url ? (
 ...
 </div> <div className="min-w-0 flex-1"> <div className="flex items-center gap-2"> <span className="text-xs text-ink/40">ด่าน {boss.stage}</span> <input
 ...
 title="HP"
 /> </div> <div className="mt-2 flex items-center gap-2"> <label className="cursor-pointer rounded-full bg-primary-tint px-3 py-1 text-xs font-medium text-primary-deep hover:bg-primary/20">

...
export default function BossBattleCamera({ boss }: { boss: Boss }) {
 ...
 return (
 <div className="flex w-full max-w-2xl flex-col items-center gap-6"> <canvas ref={sampleCanvasRef} width={32} height={18} className="hidden" />

 {/* boss */}
 <div className="relative w-full max-w-sm text-center">
 {hitFlash && (
 <div className="pointer-events-none absolute left-1/2 top-0 -translate-x-1/2"> <PopNumber value={1} prefix="-" className="text-2xl font-bold text-red-600" /> </div>
 )}
 <div
 ...
 </div> <p className="mt-2 font-display text-lg font-bold text-sun-deep">{boss.name_th}</p> <div className="mt-2 h-4 w-full overflow-hidden rounded-full bg-black/10"> <div
 className="h-full bg-gradient-to-r from-red-500 to-red-700 transition-all duration-200"
 style={{ width: `${hpPercent}%` }}
 /> </div> <p className="mt-1 text-xs text-ink/50">
 HP <PopNumber value={hp} prefix="" className="inline text-xs font-bold text-ink/70" /> / {boss.hp}
 </p> </div> <div className="glass relative aspect-video w-full overflow-hidden rounded-[24px]">
 ...
 </div> <p className="font-display text-2xl font-bold text-primary-deep">{reps} ครั้ง</p> <div className="sticky bottom-4 z-10 flex gap-4 rounded-full bg-white/70 p-2 shadow-lg backdrop-blur">
 ...
 {status === "victory" ? (
 <div className="flex gap-3"> <Link href="/boss"> <GlassButton variant="secondary">กลับไปเลือกด่าน</GlassButton> </Link> </div>
 ) : null}
 </div>

 {status === "victory" && (
 <> <Confetti trigger={true} /> <ScaleIn className="glass w-full max-w-sm rounded-[24px] p-6 text-center"> <p className="inline-block rounded-full bg-primary-tint px-4 py-1 text-sm font-medium text-primary-deep">
 ปราบ {boss.name_th} สำเร็จ!
 </p> <div className="mt-4 grid grid-cols-2 gap-4"> <div> <p className="text-xs text-ink/40">ใช้ไป</p> <p className="font-display text-3xl font-bold text-primary-deep">{reps} ครั้ง</p> </div> <div> <p className="text-xs text-ink/40">เวลาที่ใช้</p> <p className="font-display text-3xl font-bold text-ink/70">

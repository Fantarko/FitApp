...
export default async function FriendProfilePage({ params }: { params: Promise<{ id: string }> }) {
 ...
 return <main className="relative flex-1 overflow-hidden px-6 py-10"> <BlobBackground colors={["var(--color-primary)", "var(--color-plum)"]}/> <div className="mx-auto max-w-2xl"> <Link href="/friends" className="text-sm text-ink/45"> เพื่อน</Link> <FadeIn className="glass mt-4 rounded-[28px] p-6"> <div className="flex items-center gap-4">
 ...
 <div><h1 className="font-display text-2xl font-bold text-primary-deep">{profileRes.data.display_name ?? "ผู้เล่น"}</h1><p className="text-sm text-ink/50">Level {level.level} · เรตติ้ง {rating}</p></div> </div> <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
 ...
 </div> <div className="mt-5 flex flex-wrap gap-2">{badges.map(b=><span key={b.code} className="rounded-full bg-sun/15 px-3 py-2 text-sm">{b.icon} {b.name_th}</span>)}</div> <Link href="/vs" className="mt-5 inline-flex rounded-xl bg-plum px-4 py-2 text-sm font-medium text-white">ไปท้าทายใน VS</Link> </FadeIn> </div> </main>;
}

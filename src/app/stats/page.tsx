    ...
    <main className="relative flex-1 overflow-hidden px-6 py-10 md:px-10">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <FadeIn>
          ...
          <p className="mt-1 text-sm text-ink/50">สรุปผลการวิดพื้นของคุณเดือนนี้</p>
        </FadeIn>
        <FadeIn delay={0.05}>
          <ShareStatsCard
      ...
      {/* headline stats */}
      <SlideIn direction="up" delay={0.1} className="mt-6 grid gap-4 sm:grid-cols-3">
      ...
      {/* personal records */}
      {records && (
        <FadeIn delay={0.15} className="glass mt-6 rounded-[24px] p-5">
          <h2 className="font-display font-semibold text-ink">🏅 สถิติส่วนตัว (PR)</h2>
          <div className="mt-4 grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl bg-primary-tint/50 p-4 text-center">
              <p className="text-xs text-ink/50">เยอะสุดใน 1 เซสชัน</p>
              <p className="mt-1 font-display text-2xl font-bold text-primary-deep">
                {records.best_single_session_reps} ครั้ง
              </p>
            </div>
            <div className="rounded-2xl bg-plum/10 p-4 text-center">
              <p className="text-xs text-ink/50">เซสชันนานสุด</p>
              <p className="mt-1 font-display text-2xl font-bold text-plum-deep">
                {Math.floor(records.longest_session_seconds / 60)}:
                {(records.longest_session_seconds % 60).toString().padStart(2, "0")}
              </p>
            </div>
            <div className="rounded-2xl bg-sun/10 p-4 text-center">
              <p className="text-xs text-ink/50">จังหวะเร็วสุด</p>
              <p className="mt-1 font-display text-2xl font-bold text-sun-deep">

...
export default async function BossListPage() {
 ...
 {/* Header */}
 <div className="animate-fade-in mb-8 text-center"> <div className="mb-4 flex justify-center"> <span className="rounded-full bg-sun/10 px-4 py-2 text-xs font-bold tracking-wider text-sun-deep">
 BOSS MODE
 </span> </div> <h1 className="animate-slide-up font-display text-3xl font-bold text-sun-deep md:text-4xl">
 โหมดปราบบอส
 </h1> <div className="mt-4 flex justify-center gap-2"> <span className="rounded-full bg-primary-tint px-3 py-1 text-xs font-semibold text-primary-deep">ปลดล็อก {defeatedCount}/{list.length}</span> <Link href="/boss/history" className="rounded-full bg-white/70 px-3 py-1 text-xs font-semibold text-ink/60">ประวัติบอส</Link> </div> <p
 ...
 >
 {locked ? (
                <span className="text-[10px] font-medium text-ink/40">ล็อก</span>
 ) : boss.icon_url ? (
 ...
 </span> <div className="text-left"> <p className="font-display font-semibold">
 ด่าน {boss.stage}: {boss.name_th}
 </p> <p className="mt-1 text-xs text-ink/50">

...
export default function InviteJoinPage() {
 ...
 return (
 <main className="flex flex-1 items-center justify-center px-6"> <BlobBackground colors={["var(--color-plum)", "var(--color-primary)"]} /> <FadeIn className="glass w-full max-w-sm rounded-[24px] p-8 text-center">
 {state === "joining" && (
 <> <div className="mx-auto mb-4 h-8 w-8 animate-spin rounded-full border-2 border-plum/30 border-t-plum-deep" /> <p className="font-display text-lg font-bold text-plum-deep">กำลังเข้าร่วมแมตช์...</p> </>
 )}
 {state === "waiting" && (
 <> <div className="mx-auto mb-4 h-8 w-8 animate-spin rounded-full border-2 border-plum/30 border-t-plum-deep" /> <p className="font-display text-lg font-bold text-plum-deep">รอสักครู่...</p> </>
 )}
 {state === "error" && (
 <> <div className="mb-3 text-4xl"> </div> <p className="font-display text-lg font-bold text-plum-deep">{errorMsg}</p> </>
 )}
 </FadeIn> </main>
 );
}
